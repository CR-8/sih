...
interface PromptInputBoxProps {
  ...
  placeholder?: string;
  className?: string;
  mode?: 'chat' | 'ui_generation';
  onModeChange?: (mode: 'chat' | 'ui_generation') => void;
}
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Type your message here...", className, mode = 'chat', onModeChange } = props;
  ...
  React.useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (promptBoxRef.current && !promptBoxRef.current.contains(event.target as Node)) {
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            ...
            <div className="relative">
              <PromptInputAction tooltip={mode === 'chat' ? 'Chat Mode' : 'UI Generation Mode'}>
                <button
                  onClick={() => setShowModeDropdown(!showModeDropdown)}
                  className={cn(
                    "flex h-8 w-8 items-center justify-center rounded-full transition-all duration-200 text-white text-xs font-medium",
                    mode === 'chat' 
              ...
              <AnimatePresence>
                {showModeDropdown && (
                  <motion.div
                    ...
                    className="absolute bottom-full left-0 mb-2 w-32 bg-[#1F2023] border border-[#444444] rounded-lg shadow-lg z-50"
                  >
                    <button
                      onClick={() => {
                        onModeChange?.('chat');
                      ...
                      className={cn(
                        "w-full px-3 py-2 text-left text-sm hover:bg-[#2E3033] transition-colors flex items-center gap-2",
                        mode === 'chat' ? "bg-blue-500/20 text-blue-300" : "text-white/70"
                      )}
                    >
                      <span>💬</span>
                      <span>Chat</span>
                    ...
                    <button
                      onClick={() => {
                        onModeChange?.('ui_generation');
                        setShowModeDropdown(false);
                      }}
                      className={cn(
                        "w-full px-3 py-2 text-left text-sm hover:bg-[#2E3033] transition-colors flex items-center gap-2",
                        mode === 'ui_generation' ? "bg-purple-500/20 text-purple-300" : "text-white/70"
                      )}
                    >
                      <span>🎨</span>
                      <span>UI Mode</span>
