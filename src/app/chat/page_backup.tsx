...
interface ComponentPreview {
  ...
  name: string;
  category: string;
  preview: string; // Component JSX as string
}
...
function Chat({ initialMessage }: ChatProps) {
  ...
  // Function to generate inline recommendations and show them in chat
  const generateInlineRecommendations = useCallback((designData: DesignData) => {
  ...
  // Function to handle component preview
  const handleComponentPreview = useCallback((componentId: string) => {
    // Find the selected recommendation from the last message that has one
    const lastRecommendationMessage = messages.slice().reverse().find(msg => msg.selectedRecommendation);
    const selectedRec = lastRecommendationMessage?.recommendations?.find(rec => rec.id === lastRecommendationMessage.selectedRecommendation);
    
    if (selectedRec) {
      // Create URL with component and palette data
      const previewData = {
        componentId,
  ...
  const handleQuestionnaireAnswer = useCallback(async (answer: string) => {
    ...
    if (currentQuestionIndex < questionnaire.length - 1) {
      ...
      setTimeout(async () => {
        try {
          // Send design data to Flask backend
          const response = await fetch('/api/design-data', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify(newDesignData),
          });

          const completionMessage: Message = {
            id: 'questionnaire-complete',
            content: "Perfect! I've analyzed your preferences and generated personalized design recommendations for you. Here are 5 carefully curated color palettes and font combinations:",
            role: 'assistant',
            timestamp: new Date(),
          };
          ...
          const recommendationMessage: Message = {
            ...
            content: "Choose the design palette that best fits your vision:",
            role: 'assistant',
            timestamp: new Date(),
          ...
          const errorMessage: Message = {
            id: 'questionnaire-error',
            content: "I'll continue with generating recommendations using your preferences, even though the AI model connection had an issue...",
            role: 'assistant',
            timestamp: new Date(),
          };
          ...
          const recommendationMessage: Message = {
            id: 'recommendations-display',
            content: "Here are your personalized design recommendations:",
            role: 'assistant',
            timestamp: new Date(),
            isRecommendationSet: true,
            recommendations: recommendations
          ...
          setMessages(prev => [...prev, errorMessage, recommendationMessage]);
        } finally {
          setIsLoading(false);
        }
      }, 1500);
    }
  }, [currentQuestionIndex, designData, router]);

  // Function to generate inline recommendations
  const generateInlineRecommendations = useCallback((designData: DesignData): Recommendation[] => {
    ...
    const fontPairs = [
      ...
      { primary: 'Montserrat', secondary: 'Source Sans Pro' },
    ...
    return Array.from({ length: 5 }, (_, i) => ({
      id: `rec-${i + 1}`,
      name: `${designData.theme} ${i === 0 ? 'Professional' : i === 1 ? 'Modern' : i === 2 ? 'Creative' : i === 3 ? 'Elite' : 'Bold'}`,
      colors: colorPalettes[i],
      fonts: fontPairs[i],
      description: `Perfect ${designData.mood.toLowerCase()} design for ${designData.targetAudience.toLowerCase()}`,
      tags: [designData.theme.toLowerCase(), designData.mood.toLowerCase()]
    }));
  }, []);
  ...
  const handleRecommendationSelect = useCallback((recommendationId: string) => {
    ...
    const userSelectionMessage: Message = {
      id: `rec-selection-${Date.now()}`,
      content: `I've selected the ${recommendationId.replace('rec-', 'Design ')} palette.`,
      role: 'user',
      timestamp: new Date(),
    };

    // Add response with component options
    const componentMessage: Message = {
      id: 'component-display',
      content: "Excellent choice! Now you can preview how different UI components will look with your selected palette. Choose a component to preview:",
      role: 'assistant',
      timestamp: new Date(),
      isComponentSet: true,
      components: generateComponentPreviews(),
      selectedRecommendation: recommendationId
    };

    setMessages(prev => [...prev, userSelectionMessage, componentMessage]);
  }, []);

  // Function to generate component previews
  const generateComponentPreviews = useCallback((): ComponentPreview[] => {
    return [
      {
        id: 'hero-1',
        name: 'Hero Section',
  ...
  // Function to handle component selection for preview
  const handleComponentPreview = useCallback((componentId: string) => {
    if (selectedRecommendationId) {
      // Create data to pass to preview page
      const previewData = {
        componentId,
      ...
      const dataParam = encodeURIComponent(JSON.stringify(previewData));
      router.push(`/component-preview/${componentId}?data=${dataParam}`);
    }
  }, [selectedRecommendationId, designData, router]);
  ...
  const handleSendMessage = useCallback(async (message: string, files?: File[]) => {
    ...
    // Check for mode switching commands or design project detection
    const lowerMessage = message.toLowerCase();
    
    // Check if user wants to start design process
  ...
  }, [handleChatMode, chatMode, handleQuestionnaireAnswer, startQuestionnaire, handleRecommendationSelect]);
  ...
  useEffect(() => {
    if (effectiveInitialMessage && !hasProcessedInitialMessage.current && messages.length === 0) {
      ...
      const welcomeMessage: Message = {
        id: 'welcome',
        content: 'Hello! I\'m Ada, your design assistant. How can I help you today?',
        role: 'assistant',
        timestamp: new Date(),
      };
                  ...
                  {messages.map((message) => (
                    ...
                    >
                      ...
                      >
                            ...
                            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-4">
                              <ComponentTray
                                ...
                                onComponentSelect={(component: ComponentInfo) => {
                                  // Show preview button for selected component
                              ...
                              <div className="mt-3 p-3 bg-green-500/20 border border-green-500/30 rounded-lg text-center">
                                <p className="text-green-200 text-sm">
                                  ✅ Palette selected! Scroll down to preview components with your chosen design.
                                </p>
                              </div>
                            )}
                          </div>
                        ...
                        {message.isComponentSet && message.components && (
                          <div className="mt-4 space-y-3">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                              {message.components.map((comp) => (
                                <div
                                  key={comp.id}
                                  className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-4 hover:bg-white/15 transition-colors duration-200"
                                >
                                  <div className="text-center">
                                    <div className="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
                                      <span className="text-white text-lg">🎨</span>
                                    </div>
                                    <h4 className="text-white font-medium text-sm mb-1">{comp.name}</h4>
                                    <p className="text-white/60 text-xs mb-3">{comp.category}</p>
                                    <button
                                      onClick={() => handleComponentPreview(comp.id)}
                                      className="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs py-2 px-3 rounded-md transition-colors duration-200"
                                    >
                                      Preview Component
                                    </button>
                                  </div>
